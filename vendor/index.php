...
if(isset($_POST['submit']))
{
	...
	{
		$loginquery ="SELECT * FROM vendors WHERE username='$username' && password='".md5($password)."'";
...
{
	if(empty($_POST['cr_user']) || empty($_POST['cr_email']) || empty($_POST['cr_pass']) || empty($_POST['cr_cpass']) || empty($_POST['code']))
	{
		$message = "ALL fields must be fill";
	}
	else
	{	
		$check_username= mysqli_query($db, "SELECT username FROM vendors where username = '".$_POST['cr_user']."' ");
		$check_email = mysqli_query($db, "SELECT email FROM vendors where email = '".$_POST['cr_email']."' ");
		$check_code = mysqli_query($db, "SELECT adm_id FROM vendors where code = '".$_POST['code']."' ");
		...
		if(mysqli_num_rows($check_code) > 0)           // if code already exist 
		{
			...
			{
				$mql = "INSERT INTO vendors (username,password,email,code) VALUES ('".$_POST['cr_user']."','".md5($_POST['cr_pass'])."','".$_POST['cr_email']."','".$_POST['code']."')";
				mysqli_query($db, $mql);
...
<head>
    <meta charset="UTF-8">
    <title>Vendor Login | Food Ordering System</title>
...
<body>
    <div class="container">
        <div class="info">
            <h1>Hut Manager </h1><span> Login</span>
        </div>
    </div>
    <div class="form">
